...
public final class TimeUtil implements Runnable {
    ...
    private void check() {
        ...
        if (this.state == STATE.IDLE && qps.r1 > HITS_UPPER_BOUNDARY) {
            LOGGER.info("TimeUtil switches to PREPARE for better performance, reads={}/s, writes={}/s", qps.r1, qps.r2);
            this.state = STATE.PREPARE;
        } else if (this.state == STATE.RUNNING && qps.r1 < HITS_LOWER_BOUNDARY) {
            LOGGER.info("TimeUtil switches to IDLE due to not enough load, reads={}/s, writes={}/s", qps.r1, qps.r2);
